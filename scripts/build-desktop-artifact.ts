...
const resolveCustomDesktopArchiveVersion = Effect.fn("resolveCustomDesktopArchiveVersion")(function* (
  repoRoot: string,
) {
  const result = yield* spawnAndCollectOutput(
  ...
  if (archiveVersion === undefined) {
    return yield* Effect.die(
      new Error("Custom desktop builds require an exact reachable nightly release tag for SSH archives."),
    );
  }
