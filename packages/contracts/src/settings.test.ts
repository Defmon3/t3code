...
describe("ServerSettings environment theme", () => {
  it("defaults the theme and its set time to empty", () => {
    const settings = decodeServerSettings({});
    expect(settings.defaultTheme).toBe("");
    expect(settings.defaultThemeSetAt).toBe("");
  });
});
