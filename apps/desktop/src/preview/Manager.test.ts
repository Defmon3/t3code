...
describe("PreviewManager", () => {
  ...
  effectIt.effect("retries a cold hidden-tab automation snapshot capture", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        const captureCause = new Error("capture failed");
        capturePage.mockRejectedValue(captureCause);
        const failedSnapshot = yield* Effect.exit(
          manager.automationSnapshot("tab_cold_snapshot", {}),
        ).pipe(Effect.forkChild({ startImmediately: true }));
        yield* TestClock.adjust(1_000);
        const exit = yield* Fiber.join(failedSnapshot);
  ...
  effectIt.effect("releases snapshot control when a stalled guest is replaced", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        const replacementCapture = vi.fn(async () => ({
          ...
          toJPEG: () => Buffer.from("replacement-jpeg"),
          getSize: () => ({ width: 100, height: 80 }),
        }));
        fromId.mockReturnValue(makeTestPreviewWebContents(replacementCapture, 43));
        yield* manager.registerWebview("tab_1", 43);

        yield* manager.captureScreenshot("tab_1");
  ...
  effectIt.effect("types in background webviews and enables native key input", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        let focusedLookupCount = 0;
        getFocusedWebContents.mockImplementation(
          () => (focusedLookupCount++ % 2 === 0 ? previouslyFocused : webview) as never,
        );
        fromId.mockReturnValue(webview as never);
