...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(function* (
  ...
  const isCaptureTimeout = (cause: unknown): boolean =>
  ...
  const capturePageWithRetry = Effect.fn("PreviewManager.capturePageWithRetry")(function* (
    ...
    return yield* capture.pipe(
      Effect.retry({
        ...
        while: (error) => isPreviewOperationError(error) && !isCaptureTimeout(error.cause),
      }),
    );
  });
  ...
  const restorePreviouslyFocusedWebContents = Effect.fn(
    ...
    operation: "automationClick" | "automationPress",
  ) {
    if (!previouslyFocused || previouslyFocused.id === wc.id || previouslyFocused.isDestroyed()) {
      return;
    }
    const focused = yield* attempt(
      { operation: `${operation}.getFocusedWebContentsForRestore`, tabId, webContentsId: wc.id },
      () => webContents.getFocusedWebContents(),
    ).pipe(Effect.option);
    if (Option.isNone(focused) || focused.value?.id !== wc.id) return;
    yield* attempt(
      {
  ...
  const automationClick = Effect.fn("PreviewManager.automationClick")(function* (
    ...
    yield* withControlSession(tabId, wc, "click", (send) =>
      ...
      Effect.ensuring(
        restorePreviouslyFocusedWebContents(tabId, wc, previouslyFocused, "automationClick"),
      ),
    );
  });
  ...
  const performAutomationPress = Effect.fn("PreviewManager.performAutomationPress")(function* (
    ...
    const releaseInput = Effect.gen(function* () {
      ...
      yield* restorePreviouslyFocusedWebContents(tabId, wc, previouslyFocused, "automationPress");
    });
...
export class PreviewRecordingSourceSizeUnavailableError extends Schema.TaggedError<PreviewRecordingSourceSizeUnavailableError>()(
