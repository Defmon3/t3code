...
export function usePaginatedBranches(
  target: Pick<VcsRefTarget, "environmentId" | "cwd"> & { readonly prefix?: string | null },
) {
  const prefix = target.prefix?.trim() ?? "";
  const targetKey =
    target.environmentId !== null && target.cwd !== null
      ? JSON.stringify([target.environmentId, target.cwd, prefix])
      : null;
  ...
  const pageAtoms = useMemo(
    () =>
      target.environmentId !== null && target.cwd !== null
        ...
        : [],
    [cursors, prefix, target.cwd, target.environmentId],
  );
...
export function usePaginatedHistoryRefs(
  ...
  const query = target.prefix?.trim() ?? "";
