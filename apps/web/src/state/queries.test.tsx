...
import { makeVcsSnapshotCacheKey, usePaginatedHistoryRefs } from "./queries";
...
vi.mock("./vcs", () => ({
  vcsEnvironment: {
    listHistoryRefs: ({
      ...
      const result = refsState.results.get(`${generation}:${input.cursor ?? "first"}`);
      if (result === undefined)
        throw new Error(`Missing result for ${generation}:${input.cursor ?? "first"}`);
...
describe("usePaginatedHistoryRefs", () => {
  ...
  it("keeps snapshot generations distinct from revisions", () => {
    expect(makeVcsSnapshotCacheKey(1, 11)).not.toBe(makeVcsSnapshotCacheKey(2, 10));
  });
  ...
  it("uses refresh only for the first page of a refreshed ref snapshot", () => {
    ...
    expect(refsState.atoms.map((atom) => ({ ...atom.input, cacheKey: atom.cacheKey }))).toEqual([
      { cacheKey: "[0,0]", cwd: "C:/workspace", limit: 100, namespace: "local" },
      { cacheKey: "[1,0]", cwd: "C:/workspace", limit: 100, namespace: "local", refresh: true },
      { cacheKey: "[1,0]", cwd: "C:/workspace", limit: 100, namespace: "local", refresh: true },
      {
        cacheKey: "[1,0]",
        cwd: "C:/workspace",
      ...
      { cacheKey: "[1,0]", cwd: "C:/workspace", limit: 100, namespace: "local" },
    ]);
    expect(
      new Set(
        refsState.atoms
    ...
    ).toEqual(new Set(["C:/workspace:::[1,0]"]));
  });
  ...
  it("restarts from the first history refs page once when its repository revision changes", () => {
    ...
    expect(refsState.atoms.map((atom) => ({ ...atom.input, cacheKey: atom.cacheKey }))).toEqual([
      ...
      { cacheKey: "[0,1]", cwd: "C:/workspace", limit: 100, namespace: "local" },
    ]);
  });
  ...
  it("recovers an expired snapshot once per generation, including after a later refresh", () => {
    ...
    expect(refsState.atoms.map((atom) => atom.cacheKey)).toEqual(["[0,0]", "[1,0]"]);
    ...
    expect(refsState.atoms.map((atom) => atom.cacheKey)).toEqual([
      "[0,0]",
      "[1,0]",
      "[2,0]",
      "[3,0]",
