/**
 * Loading states specific to the issue surface — the first list, a search under way, and a detail
 * panel opening — use bars in the geometry of the content they stand for, pulsing on one
 * composited layer.
 *
...
/** Rows in the list's own grid — glyph, title over meta, time over comment count. */
      ...
      {Array.from({ length: rows }, (_, index) => (
        ...
        >
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            <GhostBar className="w-10 bg-muted-foreground/10" />
          </div>
        </div>
      ))}
    </div>
...
export function IssueConversationGhost({ rows = 3 }: { rows?: number }) {
  return (
    <div
      role="status"
      aria-label="Loading issue conversation"
