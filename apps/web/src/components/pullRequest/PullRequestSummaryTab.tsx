...
export function PullRequestSummaryTab({
  ...
  return (
    <div className="h-full overflow-y-auto" data-summary-scroll>
          ...
          <div className="space-y-0.5">
            {detail.linkedIssues.map((link) => (
              ...
              >
                ...
                {link.closesIssue ? (
                  <span className="shrink-0 text-muted-foreground">closed by this</span>
                ) : null}
          ...
          <div className="mt-2">
            <WorkItemMatchRows
              ...
              {...(detail.linkedIssues !== undefined && onLinkIssues
                ? { onLink: onLinkIssues, linking: pendingFinding === LINK_ISSUES_HANDOFF_KIND }
                : {})}
            />
          </div>
        )}
      ...
      >
        {activityPending ? (
          <PullRequestConversationGhost label="Loading pull request conversation" />
        ) : activityError ? (
