...
import {
  DetailGhost as PullRequestDetailGhost,
...
export function PullRequestDetailPanel({
  ...
  const startLinkIssues = (issue: WorkItemMatch) => {
    if (!detail) return;
    void startHandoff(
      "link-issues",
      buildLinkIssuesHandoff(
  ...
  if (detailQuery.isPending && !detail) {
    return <PullRequestDetailGhost label="Loading pull request" />;
  }
            ...
            {mountedTabs.has("summary") ? (
              <div className={cn("absolute inset-0", tab !== "summary" && "invisible")}>
                <PullRequestSummaryTab
                  ...
                  onLinkIssues={startLinkIssues}
                  {...(onOpenLinkedIssue === undefined ? {} : { onOpenLinkedIssue })}
