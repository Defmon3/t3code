...
/**
 * Splits a host's body into markdown runs and the uploads embedded in it, which the
 ...
 */
export function splitHostBody(body: string): ReadonlyArray<HostBodySegment> {
  const segments: HostBodySegment[] = [];
...
/** GitHub-style same-repository references for host prose, never code or authored links. */
export function remarkRepositoryAutolinks(options: { readonly repositoryUrl: string }) {
