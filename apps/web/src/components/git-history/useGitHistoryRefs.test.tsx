...
const refState = vi.hoisted(() => ({
  ...
  repositoryKey: "C:/repositories/t3code/.git" as string | null,
  isComplete: true,
  ...
  favoriteStorageKeys: [] as Array<string | null>,
  targets: [] as Array<{
...
vi.mock("../../hooks/useLocalStorage", () => ({
  useLocalStorage: (key: string | null) => {
    refState.favoriteStorageKeys.push(key);
    return [
      ...
      data:
        ...
        (options.namespace === "local" && !refState.currentRefResolved)
          ...
          : {
              ...
              repositoryKey: refState.repositoryKey,
...
import { toggleGitHistoryFavorite, useGitHistoryRefs } from "./useGitHistoryRefs";

const environmentId = EnvironmentId.make("environment-local");
const repositoryCwd = "C:/workspace";

function ref(name: string, kind: "local" | "remote" = "local"): VcsHistoryRef {
  return { current: false, isDefault: false, kind, name, worktreePath: null };
}
...
describe("useGitHistoryRefs", () => {
  beforeEach(() => {
    ...
    refState.tags = [];
    refState.favoriteBranches = [];
    refState.repositoryKey = "C:/repositories/t3code/.git";
    ...
    refState.favoriteStorageKeys = [];
    refState.targets = [];
  });

  it("persists favorites under the environment repository key", () => {
    renderRefs();

    expect(refState.favoriteStorageKeys).toEqual([
      "t3code:git-history-favorites:v1:environment-local:C:/repositories/t3code/.git",
    ]);
  });

  it("does not use a placeholder key before the repository resolves", () => {
    refState.currentRefResolved = false;

    renderRefs();

    expect(refState.favoriteStorageKeys).toEqual([null]);
  });

  it("toggles branch membership", () => {
  ...
  it("preserves ref trees and the 10k-row virtual-list model across unchanged rerenders", () => {
    ...
    refState.remote = Array.from({ length: 5_000 }, (_, index) => ref(`origin-${index}`, "remote"));
