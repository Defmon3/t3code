...
it.effect("stores only a token hash, resolves the bearer token, and revokes by thread", () =>
  Effect.gen(function* () {
    ...
    expect(resolved?.capabilities).toEqual(new Set(["preview", "pull-requests", "workspace"]));
...
it.effect(
  "always grants workspace and pull-requests and gates browser and device access independently",
  () =>
    Effect.gen(function* () {
      ...
      expect(yield* capabilitiesOf(withPreview)).toEqual(["preview", "pull-requests", "workspace"]);
      expect(yield* capabilitiesOf(withoutPreview)).toEqual(["pull-requests", "workspace"]);
      expect(yield* capabilitiesOf(withDevice)).toEqual(["device", "pull-requests", "workspace"]);
    }),
);
