...
it.layer(TestLayer, { excludeTestServices: true })("WorkspaceFileSystemLive", (it) => {
  ...
  describe("resolveFile", () => {
    it.effect("returns a workspace-relative path through a junction root", () =>
      Effect.gen(function* () {
        const workspaceFileSystem = yield* WorkspaceFileSystem.WorkspaceFileSystem;
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
    ...
    it.effect("rejects a symlinked target outside the workspace", () =>
      Effect.gen(function* () {
        ...
        const error = yield* workspaceFileSystem.resolveFile({ cwd, path: linkedPath }).pipe(Effect.flip);
    ...
    it.effect("rejects a directory", () =>
      Effect.gen(function* () {
        ...
        const error = yield* workspaceFileSystem.resolveFile({ cwd, path: directory }).pipe(Effect.flip);
