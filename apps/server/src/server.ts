...
const PlatformServicesLive = Layer.unwrap(
  Effect.gen(function* () {
    ...
    if (isWindows) {
      return BoundedChildProcessSpawner.layer().pipe(Layer.provideMerge(NodeServices.layer));
    }
...
const RuntimeCoreDependenciesLive = ReactorLayerLive.pipe(
  ...
  Layer.provideMerge(
    Layer.mergeAll(
      ...
      GitHubCli.layer,
    ).pipe(Layer.provide(GitHubGraphQlBudget.layer)),
  ),
  Layer.provideMerge(GitLayerLive),
...
export const makeRoutesLayer = Layer.mergeAll(
  ...
  McpHttpServer.layer.pipe(
    ...
    Layer.provide(WorkspaceAdoption.layer),
  ),
).pipe(
