...
const make = Effect.gen(function* () {
  ...
  const cancelTurnsAfterCompaction = Effect.fn("cancelTurnsAfterCompaction")(function* (
    ...
    const resumed = [...resumedTurnStarts.values()].filter(
      (entry) => entry.event.payload.threadId === threadId,
    );
    yield* Effect.forEach(resumed, (entry) => Deferred.succeed(entry.cancelled, undefined));
    const canceledEvents = [...queued, ...resumed.map((entry) => entry.event)].filter(
      (event, index, events) =>
        events.findIndex((candidate) => candidate.payload.messageId === event.payload.messageId) ===
        index,
    );
  ...
  const resumeTurnsAfterCompaction = Effect.fn("resumeTurnsAfterCompaction")(function* (
    ...
    while (queued.length > 0 && turnsAfterCompaction.get(threadId) === queued) {
      ...
      // In-flight replays are tracked separately so cancellation can stop their dispatch.
      queued.shift();
      ...
      resumedTurnStarts.set(commandId, { event, queued, sent, cancelled });
      const { messageId, ...request } = event.payload;
      const dispatched = yield* Effect.raceFirst(
        orchestrationEngine
          .dispatch({
