...
function mapProjectShellRow(
  ...
  return {
    ...
    skillShortcuts: row.skillShortcuts,
    skillShortcutColors: row.skillShortcutColors,
    scripts: row.scripts,
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const backgroundWorkFields = (threadId: string) => {
    const backgroundWork = threadBackgroundLiveness.getThreadBackgroundWork(threadId);
    return backgroundWork.length > 0 ? { backgroundWork } : {};
  };
                ...
                threads: Arr.filterMap(threadRows, (row) =>
                  row.deletedAt === null
                    ? Result.succeed({
                        ...
                        ...backgroundWorkFields(row.threadId),
            ...
            : repositoryIdentityResolver.resolve(option.value.workspaceRoot).pipe(
                Effect.map((repositoryIdentity) =>
                  Option.some({
                    ...
                    skillShortcuts: option.value.skillShortcuts,
                    skillShortcutColors: option.value.skillShortcutColors,
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        ...backgroundWorkFields(threadRow.value.threadId),
