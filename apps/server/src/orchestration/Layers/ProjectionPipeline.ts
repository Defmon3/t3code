  ...
  function* () {
    ...
    const refreshThreadShellSummary = Effect.fn("refreshThreadShellSummary")(function* (
      ...
      if (fields.latestUserMessageAt) {
        latestUserMessageAt = yield* projectionThreadMessageRepository.getLatestUserMessageAt({
          threadId,
        });
      }
      ...
      if (fields.pendingApprovalCount) {
        pendingApprovalCount = yield* projectionPendingApprovalRepository.countPendingByThreadId({
          threadId,
        });
      }
      ...
      if (fields.pendingUserInputCount) {
        const activities =
          yield* projectionThreadActivityRepository.listUserInputLifecycleByThreadId({ threadId });
      ...
      if (fields.hasActionableProposedPlan) {
        const actionable = yield* projectionThreadProposedPlanRepository.hasActionableByThreadId({
          threadId,
          latestTurnId: existingRow.value.latestTurnId,
        });
        hasActionableProposedPlan = actionable ? 1 : 0;
      }
        ...
        case "thread.message-sent": {
          ...
          yield* projectionThreadRepository.upsert({
            ...
            latestUserMessageAt:
              ...
              !isImportedAgentSessionMessageId(projectedMessage.value.messageId) &&
              (previousLatest === null || projectedMessage.value.createdAt > previousLatest)
                ? projectedMessage.value.createdAt
                : previousLatest,
          });
          return;
        }
    ...
    const applyProjectorForEvent = Effect.fn("applyProjectorForEvent")(function* (
      ...
      if (projectorNamesForEvent(event).includes(projector.name)) {
    ...
    const assertLiveProjectorCursors = Effect.fn("assertLiveProjectorCursors")(function* (
      ...
      if (event.sequence === 1 && currentProjectorRows.length === 0) {
        return;
      }
          ...
          yield* sql.withTransaction(
            Effect.gen(function* () {
              ...
              yield* Effect.forEach(
                projectors,
                (projector) =>
                  projectorNamesForEvent(event).includes(projector.name)
