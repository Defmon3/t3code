...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("t3-projection-cursor-batch-")))(
  ...
  (it) => {
    it.effect("writes a project, validates projector cursors, and batches cursor updates", () =>
      Effect.gen(function* () {
  ...
  it.effect("does not persist attachment files when projector transaction rolls back", () =>
    Effect.gen(function* () {
      ...
      for (const row of stateAfterFailure) {
        assert.equal(row.lastAppliedSequence, pendingEvent.sequence - 1);
      }
...
it.layer(BaseTestLayer)("OrchestrationProjectionPipeline", (it) => {
  ...
  it.effect("resumes from projector last_applied_sequence without replaying older events", () =>
    Effect.gen(function* () {
      ...
      const initialThreadCreated = yield* eventStore.append({
        type: "thread.created",
      ...
      const initialMessage = yield* eventStore.append({
        type: "thread.message-sent",
      ...
      yield* sql`
        ...
        SET last_applied_sequence = ${initialThreadCreated.sequence - 1}
      ...
      const currentProjectorNames = new Set<string>(Object.values(ORCHESTRATION_PROJECTOR_NAMES));
      const currentProjectorRows = stateRows.filter((row) =>
        currentProjectorNames.has(row.projector),
      );
      assert.lengthOf(currentProjectorRows, Object.keys(ORCHESTRATION_PROJECTOR_NAMES).length);
      for (const row of currentProjectorRows) {
