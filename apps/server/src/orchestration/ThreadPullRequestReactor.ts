...
export function isThreadPullRequestCurrent(
  ...
  return (
    Number.isNaN(threadCreatedAtMs) ||
    Number.isNaN(completedAtMs) ||
    completedAtMs >= threadCreatedAtMs
  );
}
      ...
      (group) =>
        Effect.gen(function* () {
          ...
          const plans = yield* Effect.forEach(group, (thread) =>
            Effect.gen(function* () {
              let branchPullRequest =
                detected !== null && !isThreadPullRequestCurrent(thread.createdAt, detected)
                  ? null
                  : detectedReference;
