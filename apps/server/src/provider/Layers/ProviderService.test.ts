          ...
          type === "turn.started"
            ...
            : type === "turn.completed"
              ...
              : type === "turn.aborted"
                ? { reason: "cancelled" }
                : type === "task.started"
                  ? { taskId: "task-last-seen" }
                  : type === "task.progress"
                    ? { taskId: "task-last-seen", description: "Working" }
                    : type === "task.updated"
                      ? { taskId: "task-last-seen", status: "running" }
                      : { taskId: "task-last-seen", status: "completed" };
        fanout.codex.emit({
          type,
...
describe("agent browser access", () => {
  ...
  it.effect("issues a credential without preview when agent browser access is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["pull-requests", "workspace"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("issues a credential with preview when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["device", "preview", "pull-requests", "workspace"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("drops only the preview capability when browser access alone is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["device", "pull-requests", "workspace"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("requests an MCP credential when the project overrides browser access to on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["preview", "pull-requests", "workspace"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
