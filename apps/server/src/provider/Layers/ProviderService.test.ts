...
const activityTouches: ThreadId[] = [];
const activityFanout = makeProviderServiceLayer({
  directory: {
    upsert: () => Effect.void,
    touch: (threadId) => Effect.sync(() => void activityTouches.push(threadId)),
...
activityFanout.layer("ProviderServiceLive session activity", (it) => {
  it.effect("touches bindings for lifecycle events without touching content deltas", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      const threadId = asThreadId("thread-last-seen");
      activityTouches.length = 0;
      yield* provider.startSession(threadId, {
        provider: ProviderDriverKind.make("codex"),
        providerInstanceId: codexInstanceId,
        threadId,
      ...
      yield* advanceTestClock(50);
      assert.deepEqual(activityTouches, []);

      emitEvent("turn.started", "evt-seen-turn-started");
      yield* advanceTestClock(50);
      assert.deepEqual(activityTouches, [threadId]);
