...
it.layer(NodeServices.layer)("AntigravityTextGeneration", (it) => {
  ...
  it.effect("generates structured work-item tasks and matches", () =>
    Effect.gen(function* () {
      const fixture = yield* makeFixture({
        outputs: [
          '{"prompt":"  Repair the session expiry flow.  "}',
          '{"matches":[{"candidate":2,"confidence":"high","reason":"Both report the same expiry path."}]}',
        ],
      });
      const source = {
        kind: "issue" as const,
      ...
      ).toEqual({ prompt: "Repair the session expiry flow." });
      expect(
        ...
        matches: [
          { candidate: 2, confidence: "high", reason: "Both report the same expiry path." },
        ],
      });
      const prompts = fixture.state.prompts.map((request) => {
        const content = request.prompt[0];
        return content?.type === "text" ? content.text : undefined;
      });
      expect(prompts[0]).toContain("Return a JSON object with one key: prompt.");
      expect(prompts[0]).toContain("Fix session expiry");
      expect(prompts[1]).toContain("Return JSON with one key, matches.");
      expect(prompts[1]).toContain("Session expiration regression");
      yield* fixture.assertCleaned;
    }).pipe(Effect.scoped),
  );
