...
it.effect("shares one detached discovery scan across reconnecting callers", () =>
  Effect.gen(function* () {
    ...
    return yield* Effect.gen(function* () {
      const launcher = yield* ExternalLauncher.ExternalLauncher;
      const disconnected = yield* Effect.forkChild(launcher.resolveAvailableEditors());
      yield* Deferred.await(firstProbeStarted);
      const reconnects = yield* Effect.forEach(Array.from({ length: 20 }), () =>
        Effect.forkChild(launcher.resolveAvailableEditors()),
      );
      ...
      const results = yield* Effect.forEach(reconnects, Fiber.join);
      assert.equal(
        results.every((editors) => editors.includes("vscode")),
        true,
      );
    }).pipe(
      Effect.provide(
        Layer.mergeAll(
          ...
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                PATH: "C:\\t3-editor-discovery-singleflight-test",
                PATHEXT: ".COM;.EXE;.BAT;.CMD",
              },
            }),
          ),
