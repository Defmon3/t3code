...
class GitListRefsSnapshotCacheKey extends Data.Class<{
  gitCommonDir: string;
...
class GitListRefsRefreshCacheKey extends Data.Class<{
  gitCommonDir: string;
  namespace: GitRefNamespace;
  prefix: string | null;
  worktreeRoot: string | null;
...
function parseWorktreeEntries(
  stdout: string,
): ReadonlyArray<{ readonly branchName: string | null; readonly path: string }> {
  const worktrees: Array<{ readonly branchName: string | null; readonly path: string }> = [];
...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const listRefsSnapshots = new Map<string, GitListRefsSnapshot>();
  const listRefsCursors = new Map<
    string,
    { readonly snapshotId: string; readonly offset: number }
  >();
  const newListRefsCursor = (snapshotId: string, offset: number): string => {
    const cursor = NodeCrypto.randomBytes(18).toString("base64url");
    listRefsCursors.set(cursor, { snapshotId, offset });
    return cursor;
  };
  const storeListRefsSnapshot = (snapshot: GitListRefsSnapshot): string => {
    const snapshotId = NodeCrypto.randomBytes(18).toString("base64url");
    listRefsSnapshots.set(snapshotId, snapshot);
    while (listRefsSnapshots.size > GIT_REF_SNAPSHOT_MAX_SESSIONS) {
      const oldest = listRefsSnapshots.keys().next().value;
      if (oldest === undefined) break;
      listRefsSnapshots.delete(oldest);
      for (const [cursor, value] of listRefsCursors) {
        if (value.snapshotId === oldest) listRefsCursors.delete(cursor);
      }
    }
    return snapshotId;
  };
  ...
  const bumpListRefsGeneration = (gitCommonDir: string): number =>
    setListRefsGeneration(gitCommonDir, ++listRefsGenerationSequence);
  const readListRefsSnapshot = Effect.fn("readListRefsSnapshot")(function* (
    cacheKey: GitListRefsSnapshotCacheKey,
  ) {
    const refRoot =
      cacheKey.namespace === "local"
        ? "refs/heads/"
        : cacheKey.namespace === "remote"
          ? "refs/remotes/"
          : "refs/tags/";
    const output = yield* executeGitWithStableDiagnostics(
      "GitVcsDriver.listRefs.snapshot",
      cacheKey.worktreeRoot ?? cacheKey.gitCommonDir,
      [
        "for-each-ref",
        `--count=${GIT_REF_SNAPSHOT_MAX_REFS + 1}`,
        "--sort=refname",
        "--format=%(refname)%09%(creatordate:unix)%09%(HEAD)%09%(symref)%09%(upstream:short)%09%(upstream:track)%09%(worktreepath)",
        `${refRoot}${cacheKey.prefix ?? ""}`,
      ],
      {
    ...
    for (const line of output.stdout.split("\n")) {
      ...
      if (cacheKey.namespace === "remote" && symbolicTarget.startsWith(refRoot)) {
      ...
      parsedRefs.push({
        ...
        ref: {
          name,
          current: cacheKey.namespace === "local" && headMarker === "*",
          isDefault: false,
          isRemote: cacheKey.namespace === "remote",
          ...(cacheKey.namespace === "tag" ? { isTag: true } : {}),
          worktreePath: worktreePath.length > 0 ? worktreePath : null,
          ...(cacheKey.namespace === "local" && upstreamName.length > 0
    ...
    const refs = parsedRefs.slice(0, GIT_REF_SNAPSHOT_MAX_REFS).map(({ fullName, ref }) => ({
      ...ref,
      isDefault: cacheKey.namespace === "remote" && fullName === defaultRemoteRef,
    }));
    return {
      ...
      hasPrimaryRemote:
        cacheKey.namespace === "remote" && refs.some((ref) => ref.name.startsWith("origin/")),
      isComplete,
    } satisfies Omit<GitListRefsSnapshot, "expiresAt">;
  });
  const listRefsSnapshotCache = yield* Cache.makeWith(
    (cacheKey: GitListRefsSnapshotCacheKey) =>
      Effect.yieldNow.pipe(Effect.andThen(readListRefsSnapshot(cacheKey))),
    {
  ...
  const listRefsRefreshSnapshotCache = yield* Cache.makeWith(
    (cacheKey: GitListRefsRefreshCacheKey) =>
        ...
        Effect.andThen(
          Effect.suspend(() => {
            ...
            return Cache.get(
              ...
              new GitListRefsSnapshotCacheKey({
                gitCommonDir: cacheKey.gitCommonDir,
                namespace: cacheKey.namespace,
                prefix: cacheKey.prefix,
                worktreeRoot: cacheKey.worktreeRoot,
                epoch,
              }),
            );
          }),
  ...
  const resolveListRefsSnapshot = Effect.fn("resolveListRefsSnapshot")(function* (
    ...
    while (true) {
      const generation = currentListRefsGeneration(repositoryPaths.gitCommonDir);
      const currentEpoch = listRefsEpochByCommonDir.get(repositoryPaths.gitCommonDir);
      const cacheKey = {
        gitCommonDir: repositoryPaths.gitCommonDir,
        namespace,
        prefix,
        worktreeRoot: repositoryPaths.worktreeRoot,
      };
      const snapshot =
        refresh || currentEpoch === undefined
          ? yield* Cache.get(
              listRefsRefreshSnapshotCache,
              new GitListRefsRefreshCacheKey({ ...cacheKey, generation }),
            )
          : yield* Cache.get(
              listRefsSnapshotCache,
              new GitListRefsSnapshotCacheKey({ ...cacheKey, epoch: currentEpoch }),
            );
      if (currentListRefsGeneration(repositoryPaths.gitCommonDir) === generation) {
        return {
          ...snapshot,
  ...
  const listRefs: GitVcsDriver.GitVcsDriver["Service"]["listRefs"] = Effect.fn("listRefs")(
    function* (input) {
      ...
      let offset: number;
      let snapshot: GitListRefsSnapshot;
      if (input.cursor) {
        const continuation = listRefsCursors.get(input.cursor);
        const existingSnapshot = continuation
          ? listRefsSnapshots.get(continuation.snapshotId)
          : undefined;
        ...
        listRefsCursors.delete(input.cursor);
        listRefsSnapshots.delete(continuation.snapshotId);
        snapshot.expiresAt = now + GIT_REF_SNAPSHOT_TTL_MS;
        listRefsSnapshots.set(continuation.snapshotId, snapshot);
        snapshotId = continuation.snapshotId;
        offset = continuation.offset;
      } else {
        snapshot = yield* resolveListRefsSnapshot(
          repositoryPaths,
          namespace,
          prefix,
          input.refresh === true,
        );
        snapshotId = storeListRefsSnapshot(snapshot);
        offset = 0;
      }
      ...
      return {
        ...
        nextCursor: hasMore ? newListRefsCursor(snapshotId, offset + refs.length) : null,
    ...
    if (input.cursor) {
      ...
      const matchingRefs =
        normalizedQuery === null
          ? parsedRefs
          : parsedRefs.filter(({ ref }) => ref.name.toLowerCase().includes(normalizedQuery));
  ...
  const listWorktreePaths: GitVcsDriver.GitVcsDriver["Service"]["listWorktreePaths"] = Effect.fn(
    ...
    const worktrees = parseWorktreeEntries(output.stdout).map((worktree) => ({
      ...worktree,
    ...
    return existingWorktrees.map((worktree) => worktree.path);
  });
  ...
  const invalidateRefSnapshots = Effect.fn("invalidateRefSnapshots")(function* (cwd: string) {
    ...
    for (const [cursor, continuation] of listRefsCursors) {
      if (
        listRefsSnapshots.get(continuation.snapshotId)?.gitCommonDir ===
        repositoryPaths.gitCommonDir
      ) {
        listRefsCursors.delete(cursor);
      }
    }
    for (const [snapshotId, snapshot] of listRefsSnapshots) {
      if (snapshot.gitCommonDir === repositoryPaths.gitCommonDir) {
        listRefsSnapshots.delete(snapshotId);
      }
    }
  });
