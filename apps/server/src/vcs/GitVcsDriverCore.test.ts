...
it.effect("returns paginated commit history with author, parent, and decoration details", () =>
  Effect.gen(function* () {
    ...
    const reusedCursor = yield* driver
      .getHistory({ cwd, cursor: firstPage.nextCursor, limit: 1 })
      .pipe(Effect.flip);
    assert.equal(reusedCursor._tag, "VcsSnapshotExpiredError");
...
it.effect("caps history snapshots at 1001 commits while preserving the final-page boundary", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const delegate = yield* ChildProcessSpawner.ChildProcessSpawner;
  ...
  () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const reused = yield* driver
          .listHistoryRefs({ cwd, cursor: first.nextCursor })
          .pipe(Effect.flip);
  ...
  describe("review diff previews", () => {
    ...
    it.effect("keeps untracked filenames with pathspec magic in the review", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        yield* writeTextFile(cwd, "!after.ts", "literal pathspec contents\n");
        ...
        assert.deepStrictEqual(scopedSource.files, [
          { path: "!after.ts", previousPath: null, additions: 1, deletions: 0 },
        ]);
