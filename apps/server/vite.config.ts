...
const cliVersion = process.env.APP_VERSION?.trim() || packageJson.version;
const cliBuildChannel = /^[^-+]+-(?:nightly|preview)\./.test(cliVersion) ? "nightly" : "latest";
...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __T3CODE_BUILD_VERSION__: JSON.stringify(cliVersion),
